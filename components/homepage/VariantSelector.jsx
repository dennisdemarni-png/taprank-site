  ...
  return <section className={s.purchaseSection} id="options" aria-labelledby="options-title"><div className={s.wrap}>
    ...
    <div className={s.purchaseGrid} style={{ "--variant-accent": selected.accent }}>
      ...
      <div className={s.purchaseDetails}><p className={s.selectorLabel}>Choose your stand design</p><div className={s.variantOptions} role="group" aria-label="TapRank variants">{variants.map(variant => <button type="button" key={variant.id} aria-pressed={selected.id === variant.id} data-event="variant_selected" onClick={() => onSelect(variant)} style={{ "--option-accent": variant.accent }}><span className={s.radioMark} aria-hidden="true" /><span>{variant.name}<small>£{variant.price}</small></span></button>)}</div>
        ...
        <div className={s.variantDescription} aria-live="polite" aria-atomic="true"><h3>{selected.name}{selected.id === "google" ? ` — ${googleDesign.name}` : ""}</h3><p>{selected.description}</p><div className={s.price}>£{selected.price}<span>GBP · One-off payment</span></div></div>
        {url ? <a id="purchase-checkout" className={s.button} href={url} data-event="square_checkout_click" onClick={() => homepageEvent("square_checkout_click", { variant: selected.id, ...(selected.id === "google" ? { design: googleDesign.id } : {}) })}>Buy {selected.id === "custom" ? "Custom TapRank" : selected.name} · £{selected.price} <Arrow /></a> : <button id="purchase-checkout" className={s.button} type="button" disabled aria-describedby="checkout-note">{selected.id === "google" && googleDesign.soldOut ? "Sold out" : "Checkout coming soon"}</button>}
        <p className={s.checkoutNote} id="checkout-note">{url ? "Secure payment with Square. Free UK delivery." : selected.id === "google" && googleDesign.soldOut ? "Google Review — Classic design is currently sold out." : <>Online checkout for this option will be available soon. <a href={`mailto:${TAPRANK_CONTACT.email}`}>Contact TapRank</a> for help ordering.</>}</p>
        {selected.id === "google" && googleDesign.soldOut && <button className={s.textLink} type="button" onClick={() => onDesignSelect(googleDesigns[0])}>Choose Current design — £64.99 <Arrow /></button>}
        ...
        {ETSY_URL ? <a className={s.etsyLink} href={ETSY_URL} data-event="etsy_click" onClick={() => homepageEvent("etsy_click", { variant: selected.id, ...(selected.id === "google" ? { design: googleDesign.id } : {}) })}>Prefer Etsy? Shop TapRank on Etsy <Arrow /></a> : <p className={s.etsyPending}>Prefer Etsy? Shop link coming soon.</p>}
