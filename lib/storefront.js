import { productFor } from "./commerce";
...
export function validateProductConfiguration(raw = {}) {
  ...
  if (!Number.isInteger(quantity) || quantity < 1 || quantity > 20) {
    errors.quantity = "Choose a quantity between 1 and 20.";
  }
...
export function sanitiseCartForBrowser(cart, items = []) {
  return {
    ...
    totalPence: items.reduce((total, item) => total + item.unit_price_pence * item.quantity, 0),
