...
export default function Homepage() {
  ...
  return <div className={s.home} id="top"><a className={s.skipLink} href="#main">Skip to content</a><PromotionBar />
    <header className={s.header}><div className={s.wrap}><a href="#top" aria-label="TapRank home"><Logo /></a><nav className={s.desktopNav} aria-label="Main navigation"><a href="#how-it-works">How it works</a><a href="#options">Options</a><a href="#faq">FAQ</a></nav><a className={s.navBuy} href="#options">Buy TapRank <Arrow /></a><button className={s.menuButton} type="button" aria-expanded={menuOpen} aria-controls="mobile-nav" onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? "Close" : "Menu"}</button></div>{menuOpen && <nav id="mobile-nav" className={s.mobileNav} aria-label="Mobile navigation">{[["How it works", "how-it-works"], ["Options", "options"], ["FAQ", "faq"]].map(([name, id]) => <a href={`#${id}`} key={id} onClick={() => setMenuOpen(false)}>{name}</a>)}</nav>}</header>
    <main id="main"><Hero /><TapDemo googleDesign={googleDesign} /><ActionShowcase /><CustomerProof /><VariantSelector selected={selected} onSelect={selectVariant} googleDesign={googleDesign} onDesignSelect={selectDesign} /><Ordering /><TrustStrip /><FAQ /><FinalCTA googleDesign={googleDesign} /></main>
