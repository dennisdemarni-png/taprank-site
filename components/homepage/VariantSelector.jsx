...
export default function VariantSelector({ selected, onSelect, googleDesign, onDesignSelect }) {
  ...
  return <section className={s.purchaseSection} id="options" aria-labelledby="options-title"><div className={s.wrap}>
    <div className={s.purchaseHeading}><Eyebrow>One TapRank. Make it your own.</Eyebrow><h2 id="options-title">Choose your TapRank face.</h2><p>Google Review, Instagram or Tripadvisor. The same simple tap-to-page experience.</p></div>
    <div className={s.purchaseGrid} style={{ "--variant-accent": selected.accent }}>
      ...
      <div className={s.purchaseDetails}><p className={s.selectorLabel}>Choose your TapRank face</p><div className={s.variantOptions} role="group" aria-label="TapRank faces">{variants.map(variant => <button type="button" key={variant.id} aria-pressed={selected.id === variant.id} data-event="variant_selected" onClick={() => onSelect(variant)} style={{ "--option-accent": variant.accent }}><span className={s.radioMark} aria-hidden="true" /><span>{variant.name}<small>£{variant.price}</small></span></button>)}</div>
        ...
        <a id="purchase-checkout" className={s.button} href={url} data-event="product_cta_click" onClick={() => homepageEvent("product_cta_click", { variant: selected.id, ...(selected.id === "google" ? { design: googleDesign.id } : {}) })}>Configure {selected.name} · £{selected.price} <Arrow /></a>
