...
export default function ActionShowcase() {
  ...
  return <>
    ...
    <section className={`${s.wrap} ${s.difference}`} aria-labelledby="difference-title"><div><Eyebrow>The TapRank difference</Eyebrow><h2 id="difference-title">More than<br />a review stand.</h2><p>The stand starts the connection.<br />Your TapRank page makes it useful.</p></div><div><div className={s.comparison}><div><span>A single-link stand</span><p>Tap / scan <b aria-hidden="true">→</b> One destination</p></div><div><span>With TapRank</span><p>NFC + QR <b aria-hidden="true">→</b> Your TapRank page</p><div className={s.actionBranches}><span>Reviews</span><span>Socials</span><span>Bookings</span><span>Menu & more</span></div></div></div><a className={s.differenceCta} href="/google-review-stand#configure" onClick={() => homepageEvent("product_cta_click", { variant: "google", design: "new" })}>Get Google Review TapRank · £64.99 <span aria-hidden="true">↗</span></a></div></section>
  </>;
}
