...
export function FinalCTA({ googleDesign }) {
  return <section className={s.finalSection} id="final-cta"><div className={`${s.wrap} ${s.finalGrid}`}><div><Eyebrow>Connect customers to what matters.</Eyebrow><h2>The next step<br />starts with a tap.</h2><a href="/google-review-stand" className={s.lightButton}>Shop Google Review TapRank <Arrow /></a><a href="#products" className={s.finalTextLink}>Or explore every TapRank face</a><TrustLine /></div><div className={s.finalStands}><Stand src={assets.instagram} alt="Instagram TapRank stand" /><Stand src={googleDesign.image} alt={`Google Review ${googleDesign.name} TapRank stand`} /><Stand src={assets.tripadvisor} alt="Tripadvisor TapRank stand" /></div></div></section>;
}
