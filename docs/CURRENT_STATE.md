...
Audit snapshot: **16 July 2026**

Audited base commit: `09d54e2` (`connect TapRank sales contact CTAs`)
...
The homepage is approximately 1,050 lines and the global stylesheet is approximately 5,730 lines. Much of the homepage's content, data, and interactive logic is held in one page file.

## Routes and rendering

| Route | Rendering | Current behaviour |
| --- | --- | --- |
| `/` | Static page | Marketing homepage with product sections, demos, pricing, FAQs, and TapRank sales contact routes. |
...
- Hosted demo pages include a `noindex` robots directive.
...
- Checkout or subscription billing: none found.
...
5. **Placeholder demo destinations.** Some demo actions are anchors with no matching content or generic external destinations.
6. **Unusual `/r/demo` behaviour.** It redirects away from TapRank to a generic search rather than demonstrating a TapRank page.
...
15. **Product-scope wording mismatch.** A homepage contact-email template currently asks how many “stands, cards or stickers” are needed, although current business context says cards and stickers are not active products. This is a current runtime-copy discrepancy, not changed during this documentation-only task.
...
- Existing demo URLs and the current redirect unless an approved change explicitly replaces it.
